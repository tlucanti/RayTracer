...
/**
    \brief destructor
    \detailed destructor deallocates block of GPU memory (if no other CLarray
        is referencing it)
...
/**
    ...
    \param block blocking operation flag, if set to true, function will block
        until all data will be filled
	\param size number of bytes to be dumped (size of whole array by default)
...
/**
	\brief dump GPU buffer
	\detailed same as previous `dump` method, except dumping in provided array
		`out`, without creating new vector
*/
{
...
/**
	\brief dump GPU buffer
	\detailed same as previos `dump` method, except dumping to provided pointer
		`out`, rather than vector
*/
{
    cl_int  error;
