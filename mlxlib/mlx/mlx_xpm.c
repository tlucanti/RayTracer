...
char	*mlx_int_static_line(char **xpm_data,int *pos, __attribute__((unused)) int size)
{
  static char	*copy = 0;
  static int	len = 0;
  int		len2;
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char const *file,int *width,int *height)
{
  int	fd;
  int	size;
  char	*ptr;
